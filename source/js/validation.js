...
requestForm.addEventListener("submit", function (evt) {
  ...
  //errorMessage = requestForm.querySelector(".error-message");
  checkLocalStorage();
});
...
contactsForm.addEventListener("submit", function (evt) {
  ...
  //errorMessage = contactsForm.querySelector(".error-message");
...
phoneInputs.forEach(function (item) {
  item.addEventListener("invalid", function () {
    ...
    if (item.validity.patternMismatch) {
      ...
      //добавить про неверный формат
      //errorMessage.classList.add("error-message--show");
      //console.log(item.parentElement.parentElement.querySelector("p"));
